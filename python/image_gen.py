...
def create_download_url(response_json):
    return(response_json['urls']['raw'] + "?" + ("h" if response_json["width"] > response_json["height"] else "w") + "=300&q=75&fm=png")
...
cur_img = Image.open(BytesIO(image data here))

days_until_= 10 #Please make this calculate the days until Christmas
...
#resize image
cur_img = cur_img.resize((300, 300))
...
# save image
cur_img.save(f"./images/currentImage.png")
